...
public class ConwayCubes {
    ...
    private Character nextCubeState(Coordinate coord) {
        ...
        if (cubes.getOrDefault(coord, '.') == '#') {
            ...
            throw new RuntimeException("unknown cube state: " + cubes.getOrDefault(coord, '.'));
        }
    }
    ...
    private Coordinate botLeftCoordinate() {
        Coordinate first = cubes.keySet().stream().findFirst().orElseThrow(() -> new RuntimeException("no coordinates found"));
